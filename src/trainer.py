...
class Trainer:
    def __init__(self, env, replay_buffer,
        ...
        self.optimizer = torch.optim.NAdam(
            self.model.parameters(), lr=lr, eps=1.5e-4)
        self.model.eval()
    ...
    def _compute_target(self, obs_next, rew, done):
        ...
        with torch.amp.autocast(self.device):
            ...
            if self.transform:
                max_target_q = max_target_q[:self.batch_size] + \
                    max_target_q[self.batch_size:]
                max_target_q /= 2.
    ...
    def save_explorations(self, n_episodes, save_loc='./explorations/'):
        ...
        for i in range(n_episodes):
            ...
            np.savez_compressed(
                fname, o=obs_lst, a=action_lst, r=rew_lst, d=done_lst)
    ...
    def save_models(self, prefix=''):
        ...
        if not self.no_save:
            torch.save(self.model.state_dict(),
                       self.save_loc + prefix + 'model.pt')
            torch.save(self.target_model.state_dict(),
                       self.save_loc + prefix + 'target_model.pt')
            torch.save(self.optimizer.state_dict(),
                       self.save_loc + prefix + 'optimizer.pt')
